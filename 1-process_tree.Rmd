...
```{r libaries, echo=FALSE, warnings=FALSE}
library(tidyverse)
library(R.utils)
library(ape)
library(castor)
...
## Cluster genomes
Cluster phylogenetically related genomes based on their maximum phylogenetic distances, and generate trees and relevant metadata for each cluster. 

```{r cluster_genomes, echo=FALSE}

#Generate pairwise phylogenetic distances
tipdist <- cophenetic.phylo(tree_pruned) 
#tipdist_matrix <- as.matrix(tipdist)
tipdist <- distances(tipdist, id_variable=rownames(tipdist)) %>%
    distance_matrix()

#Hierarchically cluster genomes according to phylogenetic distances
tipclust <- fastcluster::hclust(tipdist, method = "complete")
save(tipclust,file="data/tree_clust.Rdata")

#Cluster genomes

height=2.5 #threshold
...
for (c in c(1:nclusters)){
  ...
  #Generate row
  row <- tibble(cluster=c,size=tips_length,ref=ref,ehi=ehi,taxonomy=tips_taxonomy,hostn=tips_hostn,hosts=list(tips_host),genomes=list(tipvector),tree=list(subtree),meandist=meandist,maxdist=maxdist)
  #Add to table
